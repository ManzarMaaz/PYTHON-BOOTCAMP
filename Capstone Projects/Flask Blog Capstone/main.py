...
class User(UserMixin, db.Model):
    ...
    # Relationship with BlogPost (One User -> Many Posts)
    posts = relationship("BlogPost", back_populates="author")
    
    # Relationship with Comment (One User -> Many Comments)
...
class BlogPost(db.Model):
    ...
    # Relationship with Comment (One Post -> Many Comments)
    comments = relationship("Comment", back_populates="parent_post")

# 3. Create the Missing Comment Table
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = result.scalar()
        if not user:
            flash("An Account with this Email doesn't Exists !!")
            return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    comment_data = db.session.execute(select(Comment)).scalars()
    ...
    return render_template("post.html", post=requested_post, form=form, current_user=current_user, comments = comment_data)
...
def delete_comment(comment_id):
    ...
    db.session.commit()
    return redirect(url_for("show_post", post_id=comment_to_delete.parent_post.id))
